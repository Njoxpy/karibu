...
const ProductsPage = () => {
    ...
    const fetchAllProducts = async () => {
      try {
        ...
        const endpoints = [
          ...
          `${baseURL}/fresh-oil/products`,
          `${baseURL}/printing/products`,
          `${baseURL}/godown/products`,
          `${baseURL}/hardware/products`,
        ...
        const responses = await Promise.all(
          endpoints.map(async (endpoint) => {
            try {
              ...
              throw new Error(
                `Failed to fetch from ${endpoint}: ${err.message}`
              );
            }
          })
        );
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // This is a simplified example
      await axios.put(`${baseURL}/products/${selectedProduct._id}`, formData);
      setProducts(
            ...
            {paginatedProducts.length > 0 ? (
              paginatedProducts.map((product) => (
                <tr key={product._id}>
                  ...
                  <td className="border-b py-2 px-4 text-sm md:text-base">
                    {product.description}
                  </td>
