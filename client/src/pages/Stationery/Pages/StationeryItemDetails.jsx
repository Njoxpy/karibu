...
const StationeryItemDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://localhost:5000/api/v1/stationery/products/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/v1/stationery/orders', {
        method: 'POST',
        headers: {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              min="1"
            />
            {stockError && <p className="text-red-500 text-sm">{stockError}</p>} {/* Show stock error */}
          </div>
