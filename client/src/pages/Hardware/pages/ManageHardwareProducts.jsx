...
const ManageHardwareProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(
        `${baseURL}/api/v1/hardware/products/`,
        {
          headers: {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${baseURL}/api/v1/hardware/products/${editProduct._id}`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${baseURL}/api/v1/hardware/products/${selectedProduct._id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6 text-blue-500">Manage Products</h1>
      <input
        type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {currentItems.map((product) => (
          <div key={product._id} className="border p-4 rounded">
            <img
              src={product.image ? `${baseURL}${product.image}` : "/default-image.jpg"}
              alt={product.name}
...
export default ManageHardwareProducts;