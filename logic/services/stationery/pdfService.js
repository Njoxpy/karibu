...
const generateStationeryPDF = (orders, res) => {
  try {
    ...
    const doc = new PDFDocument({
      ...
      autoFirstPage: true,
      info: {
        Title: `Stationery Sales Report - ${moment().format("YYYY-MM-DD")}`,
        Author: "Stationery Management System",
        Subject: "Sales Report",
        Keywords: "stationery, sales, report",
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=stationery-report-${moment().format("YYYY-MM-DD")}.pdf`
    );
...
const addHeader = (doc, colors) => {
  ...
  doc
    ...
    .fontSize(22)
    .font("Helvetica-Bold")
    .text("STATIONERY SALES REPORT", 50, 54, { align: "center" })
    .moveDown(2);
};
