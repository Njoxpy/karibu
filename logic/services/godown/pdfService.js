...
const generateGodownPDF = (orders, res) => {
  ...
  // Add main heading
  doc.fontSize(24)
     .text("Godown Sales Report", { align: "center" })
     .moveDown(2);
  ...
  orders.forEach(order => {
    doc.fontSize(12);
    // Order ID
    doc.text(order.orderId || "N/A", orderIdX, doc.y);
    
    // Quantity (right-aligned)
    doc.text(order.quantity?.toString() || "0", 
             quantityX, 
             doc.y - 12, 
             { align: "right", width: 30 });
    
    // Total Price (right-aligned)
    doc.text(order.totalPrice?.toFixed(2) || "0.00", 
             priceX, 
             doc.y - 12, 
  ...
  doc.text(`Total Revenue: Tsh`, 
           400, 
           doc.y, 
           { continued: true });
  doc.text(totalRevenue.toFixed(2), 
           { align: "right" });
